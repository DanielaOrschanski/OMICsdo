...
setwd("/media/4tb2/Daniela/Environ/FusionesActualizado/Tiroides/CarcinomasOncociticos-PRJNA445446/SRR6888826/trimmed")
library(readxl)
...
for (fusion in 1:nrow(fusions)) {
  ...
  yBreakpointLabels <- ifelse(alignmentsFile != "", 0.8, 0.8)
  #yCoverage <- 0.72
  yCoverage <- 0.4
  #yExons <- 0.67
  yExons <- 0.3
  ...
  text(breakpoint1+0.05, yBreakpointLabels-0.02, paste0("bp1\n", fusions[fusion,"display_contig1"], ":", fusions[fusion,"breakpoint1"]), adj=c(1,0), cex=fontSize*0.9)
  ...
  text(gene2Offset+breakpoint2-0.05, yBreakpointLabels - 0.05, paste0("bp2\n", fusions[fusion,"display_contig2"], ":", fusions[fusion,"breakpoint2"]), adj=c(0,0), cex=fontSize*0.9)
  ...
  # Dibujá los nombres de los exones en el grafico coverage ------------------------
  exon_x <- (exons1$left + exons1$right) / 2
  label_y <- yCoverage - 0.02
  text(exon_x, label_y, labels = exons1$exonNumber, cex = 1, srt = 0, adj = 1)

  exon_x <- (exons2$left + exons2$right) / 2
  label_y <- yCoverage - 0.03
  text(gene2Offset + exon_x, label_y, labels = exons2$exonNumber, cex = 1, srt = 0, adj = 1)
